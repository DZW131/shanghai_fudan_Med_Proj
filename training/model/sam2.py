...
class SAM2Train(SAM2Base):
    ...
    def track_step(
        ...
        # current_out["multistep_pred_masks"] = sam_outputs.low_res_masks
        # current_out["multistep_pred_masks_high_res"] = sam_outputs.high_res_masks
        # current_out["multistep_pred_multimasks"] = [sam_outputs.low_res_multimasks]
        current_out["multistep_pred_multimasks_high_res"] = [
            sam_outputs.high_res_multimasks
        ]
        current_out["multistep_pred_ious"] = [sam_outputs.ious]
        # current_out["multistep_point_inputs"] = [point_inputs]
        current_out["multistep_object_score_logits"] = [sam_outputs.object_score_logits]
        ...
        if (
            ...
            point_inputs, final_sam_outputs = self._iter_correct_pt_sampling(
                ...
                high_res_features,
                pix_feat,
                sam_outputs.low_res_multimasks,
                sam_outputs.high_res_multimasks,
                sam_outputs.ious,
                sam_outputs.low_res_masks,
                sam_outputs.high_res_masks,
                sam_outputs.object_score_logits,
                current_out,
            )

            sam_outputs.low_res_masks = final_sam_outputs.low_res_masks
            sam_outputs.high_res_masks = final_sam_outputs.high_res_masks
            sam_outputs.obj_ptr = final_sam_outputs.obj_ptr
            sam_outputs.object_score_logits = final_sam_outputs.object_score_logits

        # Use the final prediction (after all correction steps for output and eval)
        current_out["pred_masks"] = sam_outputs.low_res_masks
        current_out["pred_masks_high_res"] = sam_outputs.high_res_masks
        current_out["obj_ptr"] = sam_outputs.obj_ptr
    ...
    def _iter_correct_pt_sampling(
        ...
        for _ in range(self.num_correction_pt_per_frame):
            ...
            new_points, new_labels = get_next_point(
                gt_masks=gt_masks[:, :, self.multitask_num - 1, :, :],
                pred_masks=(
                    pred_for_new_pt[:, self.multitask_num - 1, :, :].unsqueeze(1)
            ...
            all_pred_masks.append(sam_outputs.low_res_masks)
            all_pred_high_res_masks.append(sam_outputs.high_res_masks)
            all_pred_multimasks.append(sam_outputs.low_res_multimasks)
            all_pred_high_res_multimasks.append(sam_outputs.high_res_multimasks)
            all_pred_ious.append(sam_outputs.ious)
            all_point_inputs.append(point_inputs)
            all_object_score_logits.append(sam_outputs.object_score_logits)
...
class SAM2Export(SAM2Train):
    def __init__(self, *args, **kwargs):
        ...
        self.preprocess = nn.Sequential(Normalize(self.mean, self.std))

    def forward_encoder(self, input_image):
        ...
        #     "image_embed": feats[2],
        # }
        return self.feats[0], self.feats[1], self.feats[2]
        # 处理prompt
    ...
    def forward_decoder(
        ...
        high_res_features = [
            ...
            for feat_level in [
                (
                    high_res_feats_0
                    if high_res_feats_0 is not None
                    else self.feats[0]
                ),
                (
                    high_res_feats_1
                    if high_res_feats_1 is not None
                    else self.feats[1]
                ),
            ]
        ]
        _low_res_masks, _iou_predictions = [], []
        for decoder in self.sam_mask_decoders[:]:
            low_res_masks, iou_predictions, _, _ = decoder(
                image_embeddings=(
                    image_embed[img_idx].unsqueeze(0)
                    if image_embed is not None
                    else self.feats[2][img_idx].unsqueeze(0)
                ),
            ...
            _low_res_masks.append(low_res_masks)
            _iou_predictions.append(iou_predictions)
        # masks = self._transforms.postprocess_masks(
        #     low_res_masks, (1024,1024)
        # )
        low_res_masks = torch.cat(_low_res_masks, dim=1)
        iou_predictions = torch.cat(_iou_predictions, dim=1)
        low_res_masks = torch.clamp(low_res_masks, -32.0, 32.0)
        # masks = masks > 0

        return low_res_masks, iou_predictions

    def forward(self, input_image, point_coords, point_labels):
        self.forward_encoder(input_image)
